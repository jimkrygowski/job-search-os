...
4. Run all three tiers in parallel, then summarize — cross-check findings against `tracker.md` and flag any mismatches (e.g. a "next action" already resolved, a stage that's stale).
...
3. Only if that reveals genuinely new messages, call `get_thread` with `messageFormat: FULL_CONTENT`. If the result exceeds the token limit and gets saved to a file, run it through `tools/gmail_extract.py` (from the job-search repo root) instead of hand-parsing HTML/quoted history:
   ```
