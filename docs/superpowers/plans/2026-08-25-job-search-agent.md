...
class TrackerCLITest(unittest.TestCase):
    ...
    def test_close_moves_row_to_closed_and_writes_notes(self):
        ...
        notes = Path("opportunity/Acme/VP Engineering/notes.md").read_text()
...
Find:
```
You are Jim's career coach. Jim is an engineering executive targeting CTO, VP Engineering, or Senior Director roles. He was recently laid off and is actively searching.
```
...
Find:
```
1. **Read the pipeline:** `~/code/job-search/companies/` and `~/code/job-search/00_Admin/applications.csv`
2. **Read memory context:** Check the Job Search Project memory for current pipeline state
...
- Has done both large-org and growth-stage engineering leadership — comfortable at both
- AI infrastructure work is a differentiator; wants to continue in that direction
...
```
from:(contact1@example-recruiter.example OR contact2@example-search-firm.example OR contact3@example-company.example) newer_than:1d
```
```
...
- **LinkedIn:** `from:(jobalerts-noreply@linkedin.com OR jobs-listings@linkedin.com OR jobs-noreply@linkedin.com) newer_than:2d` — emails route to a personal Gmail alias

For ZenSearch: extract each job listing (title, company, location, comp). Flag any that match the user's target profile — role, industry, location, and comp floor as defined in `trajectory.md`. Skip the rest.
...
Run: `grep -niE '<full-name>|<personal-domain>|<job-alert-domain>|applications\.csv|00_Admin' .claude/skills/morning-scan/SKILL.md`
Expected: no matches (exit code 1)
