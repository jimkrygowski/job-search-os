# Offer Negotiator — Bucket 5: Moments 3-4 (Counter-Negotiation Planning, Final Accept/Decline) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add the last two of the offer-negotiator skill's four negotiation moments — Counter-Negotiation Planning (§6.3) and Final Accept/Decline (§6.4) — to `.claude/skills/offer-negotiator/SKILL.md`, completing the skill. This is the last bucket of the feature.

**Architecture:** One file to touch this time: `research.md`'s moment index (added in Bucket 4) already has correct rows for moments 3 and 4 — verified directly against the file, no changes needed there. `SKILL.md` gets two more full-file replacements: Task 1 restructures the dispatch's "offer in hand" branch into its first two sub-choices (Offer Breakdown, Counter-Negotiation Planning) and adds `## Counter-Negotiation Planning`; Task 2 adds the third sub-choice (Final Accept/Decline) and the `## Final Accept/Decline` section, which is a pure hand-off to the already-unmodified `career-coach` skill (spec §6.4: "Zero changes required to `career-coach.md` itself"). Per Bucket 4's final-review forward guidance (recorded in project memory), this plan bakes its own "docs surfaces" fix directly into Task 2 — updating `README.md`'s `offer-negotiator` row to describe the complete, finished skill — rather than leaving it for a final-review fix wave to catch a second time.

**Tech Stack:** Prose only, not unit-tested, consistent with spec §9 and every prior bucket's skill-file work. No new Python code or tests — `option_value.py` (Bucket 2) is untouched, and Counter-Negotiation Planning re-derives no equity math, it only reads the Offer Breakdown's already-computed numbers back out of `notes.md`.

**Spec:** `docs/superpowers/specs/2026-09-01-offer-negotiator-design.md` — §6.3 (Counter-Negotiation Planning), §6.4 (Final Accept/Decline hand-off), §8 (no new guardrails beyond CLAUDE.md #1/#2).

## Global Constraints

- **Counter-Negotiation Planning synthesizes three inputs** (spec §6.3): the opportunity's Offer Breakdown (from `notes.md`), `state/career/comp_target.md`'s walk-away numbers/priorities, and `research.md`'s tactics — never a generic "counter 10-15%" template untethered from any of the three.
- **Final Accept/Decline does not run its own decision session** (spec §6.4): it only (1) confirms the opportunity's `notes.md` has a sourced comp summary, and (2) explicitly invokes `career-coach`, which already scores "Compensation & upside" in its Evaluation Template (`career-coach/SKILL.md:66,125`) and already reads `notes.md` in its Session Start Protocol (`career-coach/SKILL.md:26`) — both verified directly against that file, not assumed. **`career-coach/SKILL.md` itself is never modified by this bucket.**
- **The dispatch's "offer in hand" branch must be re-cut into sub-choices, not appended to** (per Bucket 4's own final-review forward guidance, recorded in project memory): understand it → Offer Breakdown, counter it → Counter-Negotiation Planning, decide on it → Final Accept/Decline. This was a known, planned restructuring going into this bucket, not a defect to discover in review.
- **`research.md`'s moment index already has correct rows for moments 3 and 4** — verified directly (`research.md:29-30`): row 3 lists exactly Integrative (Multi-Issue) Negotiation, Gender and Framing Effects in Salary Negotiation, and Deadline and Pressure Tactics; row 4 says "None." Neither row needs editing in this bucket.
- **Counter-Negotiation Planning's tactics carry the same evidentiary cautions `research.md` states for them** — verified directly against the file's "How to use correctly" notes (`research.md:264-270,359-371,447-459`): (1) Integrative — ask for the user's cash/equity/benefits priority before suggesting trade language if `comp_target.md` doesn't have one; (2) Gender and Framing Effects — never assert gender predicts negotiation style as a general rule, but take a user's stated backlash concern seriously and note that a market-data-backed rationale is consistent with reducing that risk; (3) Deadline and Pressure Tactics — coach a firm, calm response to an artificial deadline, and never cite a specific turnover/retention statistic for exploding offers since `research.md` found no rigorously sourced field figure for that claim.
- **Never invent a counter number, offer number, or comp fact** — CLAUDE.md guardrail #1, applied the same way it already is in Offer Breakdown: every number traces to `comp_target.md`, the Offer Breakdown's own output, or the user's stated conversation.
- **Opportunity folders are always resolved via the tool**: `python3 tools/tracker.py opportunity-path "<Company>" "<Role>"` (same convention as every other moment and sibling skill).
- Follow the existing file's established conventions exactly: `**Bold label** — description` for list items, `### <Heading>` for a moment's internal subsections, `<date>` placeholders in backticks for output headings (`## Counter-Negotiation Plan (<date>)`, matching `## Offer Breakdown (<date>)`/`## First-Contact Prep (<date>)`).

---

### Task 1: `SKILL.md` — dispatch re-cut + Counter-Negotiation Planning

**Files:**
- Modify: `.claude/skills/offer-negotiator/SKILL.md` (full-file replacement — see Step 1)

**Interfaces:**
- Consumes: nothing new from earlier buckets beyond what's already in the file (reads the Offer Breakdown's `## Offer Breakdown (<date>)` notes.md convention established in Bucket 4, and `comp_target.md`'s fields established in Bucket 3 — both by description, not literal coupling).
- Produces: the dispatch's first two "offer in hand" sub-choices, which Task 2 extends with a third.

**Verify (no automated test — prose file, consistent with spec §9):** after writing, confirm: (a) the frontmatter `description:` and `## Purpose` now describe four capabilities (Setup Mode, First-Contact Prep, Offer Breakdown, Counter-Negotiation Planning) — no mention of Final Accept/Decline yet; (b) `## Session Start`'s dispatch has exactly two sub-choices under the "offer in hand" branch, both routing to sections that exist in this file; (c) all of Bucket 4's Setup Mode, First-Contact Prep, and Offer Breakdown content is present and unchanged; (d) `## Counter-Negotiation Planning`'s "What to Produce" items each cite the specific research.md subsection they're grounded in, matching this task's Global Constraints; (e) `## Guardrails` has exactly two new bullets beyond Bucket 4's four.
...
git commit -m "Add offer-negotiator Counter-Negotiation Planning moment"
```

---

### Task 2: `SKILL.md` — Final Accept/Decline + README completion
...
**Interfaces:**
- Consumes: Task 1's dispatch structure and `## Counter-Negotiation Planning`/`## Guardrails` sections (extends them, doesn't rewrite their substance).
- Produces: nothing consumed elsewhere — this is the last task of the last bucket.

**Verify (no automated test — prose file, consistent with spec §9):** after writing, confirm: (a) the frontmatter `description:` and `## Purpose` now describe all four moments plus Setup Mode; (b) `## Session Start`'s dispatch has exactly three sub-choices under the "offer in hand" branch; (c) all of Task 1's content is present and unchanged; (d) `## Final Accept/Decline` explicitly invokes `career-coach` and does not attempt its own scoring/decision logic; (e) `## Guardrails` has exactly one new bullet beyond Task 1's six; (f) `README.md`'s `offer-negotiator` row describes the complete, finished skill (all four moments), not just Offer Breakdown.
...
- [ ] **Step 2: Update `README.md`'s `offer-negotiator` row**

The row currently reads (`README.md:25`):
```
...
Read `SKILL.md` back and confirm the six points in this task's Verify section. Read `README.md`'s updated row back and confirm it mentions all four moments.

- [ ] **Step 4: Commit**

```bash
git add .claude/skills/offer-negotiator/SKILL.md README.md
git commit -m "Add offer-negotiator Final Accept/Decline moment, complete the skill"
```
